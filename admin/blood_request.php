	...
	<table width="100%" cellpadding="5" cellspacing="0" bordercolor="#ccc" style="text-align:center; font-family:Lucida Sans Unicode, Lucida Grande, sans-serif; font-size:14px;" border="1">
       <tr>
         ...
         <td width="20%" align="center" bgcolor="#F7F7F7" style=" border-bottom:none;"><p>Required On</p></td>  
          <td width="20%" align="center" bgcolor="#F7F7F7" style=" border-bottom:none;"><p>Status</p>
      <td width="20%" align="center" bgcolor="#F7F7F7" style=" border-bottom:none;"><p>Action</p>
          </td>
...
<?php	  
		...
		$query = "SELECT * FROM blood_request ORDER BY request_requiredon ASC" ; //LIMIT $offset,$rec_limit "; 
		...
		while(($row = mysql_fetch_array($result)))
      {
			...
			echo '<td><span style="font-size:15px;">'.$row['request_requiredon'].'</span> </td>';
			echo '<td><span style="font-size:15px;">'.$row['request_status'].'</span> </td>';
			if($row['request_status']=="OPEN"){
echo '<td style="align:center">
			<a href="closerequest.php?id='.$row['request_id'].'">
			<input type="button"  style="width:75px;margin-top:3px;background-color:white;border:1px solid rgba(201, 33, 60, 0.62);" value="Close" name="status_close" id="status_close" /></a>
			</td></tr>'; }
